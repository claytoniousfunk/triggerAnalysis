...
void triggerAnalysisSimple_CsJetsEta2p1_ppRef(std::string triggerFile = "openHLTFiles_ppRef_merge.root",
                           TString input = "root://cmsxrootd.fnal.gov//store/user/katatar/run3/run3_pp22_privateMC_QCDDijet15_pythia8_5p36TeV_FOREST_mAOD/CRAB_UserFiles/crab_run3_pp22_privateMC_QCDDijet15_pythia8_5p36TeV_FOREST_mAOD_userInput/220919_075212/0000/HiForestMiniAOD_1.root",
                           TString output = "rootFiles/out_CsJetsEta2p1_ppRef.root"){
 ...
 //   treeggHiNtuplizer = (TTree*)fileTmp->Get(treePath.c_str());
	...
	if(pthat < 0.35*maxPt_denom) continue;
        ...
        if(fabs(maxEta_denom)>etaCut) continue; // skip event if the leading jet is outside eta range

	    if(maxPt_denom > 0) {
            
            denom->Fill(maxPt_denom,weight);
            if(fabs(maxPt_denom) > 0) eta_denom->Fill(maxEta_denom,weight);
        ...
        if(triggerDecision_60) {// only fill the numerator if the trigger is on.
            ...
            if(maxPt_num > 0){
                ...
                if(maxPt_num > 0) eta_num_60->Fill(maxEta_num,weight);
        ...
        if(triggerDecision_80) {// only fill the numerator if the trigger is on.
            ...
            if(maxPt_num > 0){
                ...
                if(maxPt_num > 0) eta_num_80->Fill(maxEta_num,weight);
                //std::cout << "maxPt_num = " << maxPt_num << std::endl <<  std::endl;
            } 
        ...
        if(triggerDecision_100) {// only fill the numerator if the trigger is on.
            ...
            if(maxPt_num > 0){
                ...
                if(maxPt_num > 0) eta_num_100->Fill(maxEta_num,weight);
                //std::cout << "maxPt_num = " << maxPt_num << std::endl <<  std::endl;
            } 
        ...
        if(triggerDecision_120) {// only fill the numerator if the trigger is on.
            ...
            if(maxPt_num > 0){
                ...
                if(maxPt_num > 0) eta_num_120->Fill(maxEta_num,weight);
                //std::cout << "maxPt_num = " << maxPt_num << std::endl <<  std::endl;
            } 
    ...
    leg->AddEntry(r_60,"HLT_HICsAK4PFJet60Eta2p1_v","p");
    leg->AddEntry(r_80,"HLT_HICsAK4PFJet80Eta2p1_v","p");
    leg->AddEntry(r_100,"HLT_HICsAK4PFJet100Eta2p1_v","p");
    leg->AddEntry(r_120,"HLT_HICsAK4PFJet120Eta2p1_v","p");
