...
public :
    ...
    static int setBranchAdressTrigger(TTree* tree, std::string branch, bool & branchVal);
    static void setBranchesTrigger(TTree* tree, std::vector<std::string> branchNames, bool val[], int nVal);

    static int setBranchAdressTriggerPrescl(TTree* tree, std::string branch, float & branchVal);
    static void setBranchesTriggerPrescl(TTree* tree, std::vector<std::string> branchNames, float val[], int nVal);
    int nBinsX;
...
int triggerAnalyzer::setBranchAdressTrigger(TTree* tree, std::string branch, bool & branchVal)
{
...
void triggerAnalyzer::setBranchesTrigger(TTree* tree, std::vector<std::string> branchNames, bool val[], int nVal)
{
...
int triggerAnalyzer::setBranchAdressTriggerPrescl(TTree* tree, std::string branch, float & branchVal)
{
    if (branch.size() == 0 || branch == CONFIGPARSER::nullInput) {
        branchVal = 1;
        return -2;
...
void triggerAnalyzer::setBranchesTriggerPrescl(TTree* tree, std::vector<std::string> branchNames, float val[], int nVal)
{
    for (int i = 0; i < nVal; ++i) {
        int branchSetFlag = triggerAnalyzer::setBranchAdressTriggerPrescl(tree, branchNames[i], val[i]);
        if (branchSetFlag == -1) {
            std::cout << "set branch addresses for triggers" << std::endl;
            std::cout << "Following branch is not found : "  << branchNames[i].c_str() << std::endl;
        }
